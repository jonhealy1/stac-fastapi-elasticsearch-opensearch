...
from stac_fastapi.core.validate import (
    async_validate_stac,
    async_validate_with_fast_validator,
)
...
class TransactionsClient(AsyncBaseTransactionsClient):
    ...
    async def _validate_feature_collection(
        ...
        if get_bool_env("ENABLE_STAC_VALIDATOR") and not skip_validation:
            return await async_validate_with_fast_validator(features)
        return features, {}
    ...
    async def _create_feature_collection(
        ...
        **kwargs,
        ...
        # 1. DEDUPLICATION LAYER + COLLECTION INJECTION
        seen_ids: dict = {}
        for feature in raw_features:
            feature_id = feature.get("id")
            if feature_id is not None:
                # Inject collection ID if not present
                if "collection" not in feature:
                    feature["collection"] = collection_id
                    logger.debug(
                        f"Injected collection '{collection_id}' into item {feature_id}"
                    )
        ...
        logger.info(
            f"Input features: {len(raw_features)}, Unique features after dedup: {len(unique_features)}, Skipped: {skipped_batch_duplicates}"
        )
        if skipped_batch_duplicates > 0:
            logger.info(f"Unique feature IDs: {list(seen_ids.keys())}")
...
class BulkTransactionsClient(BaseBulkTransactionsClient):
    ...
    def bulk_item_insert(
        ...
        Returns:
            ...
            On error, raises HTTPException with detailed error information.
        """
        request = kwargs.get("request")
        ...
        # 2. VALIDATION LAYER (Use FastValidator for efficiency)
        validation_errors: dict[str, list[str]] = {}
        validation_error_count = 0
        if get_bool_env("ENABLE_STAC_VALIDATOR"):
            from stac_fastapi.core.validate import validate_with_fast_validator

            valid_items, validation_errors = validate_with_fast_validator(unique_items)
        ...
        # Format conflict errors for response
        def format_conflict_errors(conflicts):
            conflict_details = {}
            for c in conflicts:
                doc_id = next(iter(c.values())).get("_id", "")
        ...
        total_skipped = (
            skipped_batch_duplicates + len(conflict_errors) + validation_error_count
        )

        # Build response with validation, conflict and error details
        response: dict = {
            "message": f"Successfully added/updated {success} Items. {total_skipped} skipped ({skipped_batch_duplicates} duplicates, {validation_error_count} validation errors, {len(conflict_errors)} conflicts). {len(other_errors)} errors occurred."
        }
        if validation_errors:
            response["validation_errors"] = validation_errors
        if conflict_errors:
        ...
        # If no items were successfully added, return as error response
        if success == 0:
            raise HTTPException(
                status_code=400,
                detail=response,
